...
VOID regread(UINT32 regindex, UINT64* inscount)
{
	...
	{
		UINT64 rawdependancy = *inscount - raw[regindex].write;
		//stats->_rawdep += dependancy;
		//cout << "adding " << dependancy << " instructions; total is now " << stats->_rawdep << endl;
		if (rawdependancy <= 2)
			stats->_rawdep02++;
...
{
	UINT64 wawdependancy, wardependancy;
	if (raw[regindex].write != 0)
	{
...
VOID Fini(INT32 code, VOID *v)
{
	...
	avgbblsize = 1.0 * stats->_insbblcount / stats->_bblcount;
